...
/** This class represents a real-coded steady generational GA. This GA uses binary tournament selection, elitist steady-generational replacement, BLX-alpha crossover, and Gaussian mutation. Steady-generational replacement replaces a random individual with the newly created child.
 * 
 * @since 1.0
 * @author Aaron Garrett
 * @version 1.0
 */
public class RealSteadyGenerationalGA {
	 ...
	 */
	public RealSteadyGenerationalGA(int populationSize, double[] min, double[] max, double crossoverUsageRate, double blxAlpha, double mutationUsageRate, double mutationRate, double mutationRange, EvaluationFunction ef, TerminationCriteria tc, ECMonitor ecEval) {
		...
		SteadyGenerationalSelection stdyGenSelection = new SteadyGenerationalSelection(1);
		...
		population = new Population(populationSize, bounds, ef, tournSelection, blxRecombination, gaussMutOp, stdyGenSelection, noMig); 
	}
	 ...
	 */
	public RealSteadyGenerationalGA(int populationSize, double[] min, double[] max, double crossoverUsageRate, double blxAlpha, double mutationUsageRate, double mutationRate, double mutationRange, EvaluationFunction ef, TerminationCriteria tc, ECMonitor ecEval, MigrationOperator migOp) {
		terminationCriteria = tc;
		ecEvaluator = ecEval;
		numDimensions = min.length;
		Interval[] bounds = new Interval[numDimensions];
		...
		//		         Population(int           , Interval[], EvaluationFunction, ParentSelection, RecombinationOperator, MutationOperator, SurvivorSelection, MigrationOperator)
		population = new Population(populationSize, bounds, ef, tournSelection, blxRecombination, gaussMutOp, stdyGenSelection, migOp); 
	}
