...
public class RealSteadyStateUniformGA {
	 ...
	 */
	public RealSteadyStateUniformGA(int populationSize, double[] min, double[] max, double crossoverUsageRate, double mutationUsageRate, double mutationRate, double mutationRange, EvaluationFunction ef, TerminationCriteria tc, ECMonitor ecEval) {
		...
		population = new Population(populationSize, bounds, ef, tournSelection, uniRecombination, gaussMutOp, stdyStSelection, noMig); 
	}

	/** This constructor creates an instance of the GA based on the population size, the array of minimum bounds for the genes, the array of maximum bounds for the genes, the crossover usage rate, the mutation usage rate, mutation rate, the mutation range, evaluation function, termination criteria, EC evaluator, and migration operator.
	 * 
	 * @since 1.0
	 * @param populationSize The size of the population
	 * @param min The array of minimum bounds for the genes
	 ...
	 */
	public RealSteadyStateUniformGA(int populationSize, double[] min, double[] max, double crossoverUsageRate, double mutationUsageRate, double mutationRate, double mutationRange, EvaluationFunction ef, TerminationCriteria tc, ECMonitor ecEval, MigrationOperator migOp) {
		terminationCriteria = tc;
		ecEvaluator = ecEval;
		numDimensions = min.length;
		Interval[] bounds = new Interval[numDimensions];
		...
		//		         Population(int           , Interval[], EvaluationFunction, ParentSelection, RecombinationOperator, MutationOperator, SurvivorSelection, MigrationOperator)
		population = new Population(populationSize, bounds, ef, tournSelection, uniRecombination, gaussMutOp, stdyStSelection, migOp); 
	}
