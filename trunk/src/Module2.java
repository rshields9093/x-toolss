...
class Module2 implements Cloneable
{
	...
	// public boolean hasAllFields(){
			...
			if(!filePath.equals("")){
				try { 
					...
					while ((line = br.readLine()) != null) { 
						...
						tokenCount = 0;
						line = line.trim();
						if(line.trim().equals("") || line.startsWith(comment)){//ignores commented / empty lines
							continue;
						}
						...
						while(st.hasMoreTokens()){
							...
							if(token.charAt(0) == '#' || token.startsWith("//")){ //comments
								prevLine = line;
								break;
							}
							tokenCount++;

							if(!state.equals("") && !state.endsWith(":")){//check for colon at end of state declaration
								if(!token.equals(":")){
									if(tokenCount > 1){
							...
							if (state.equals("") && nextState.equals("")){//check for no state declared
								errorReport = "     -ERROR (syntax - no state declared, ex. input, input file, codefile, output, or output file) line " + lineCount + ": \n          " + line;
								prevLine = line;
								break;
							}
				...
				System.err.println("Error: Can't check syntax before file fileName has been given.");
			}
		...
		private String nextState(String mainState, String minState, String token){
			//check states and adds all variables / files to module
		...
		public static boolean isVar(String var){// determines whether this is a valid variable fileName 
			...
			if(length > 25){
				...
				for(int i = 0; i < length; i++){
					c = var.charAt(i);
					//Only accept the following characters
