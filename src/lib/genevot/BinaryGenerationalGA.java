...
/** This class represents a binary-coded generational GA. This GA uses binary tournament selection, generational replacement (with possible elitism), uniform crossover, and bit-flip mutation. For a detailed treatment of GAs, see G. Dozier, A. Homaifar, E. Tunstel,  and D. Battle, "An Introduction  to Evolutionary Computation" (Chapter 17), Intelligent Control Systems Using Soft Computing Methodologies, A. Zilouchian & M. Jamshidi (Eds.), pp. 365-380, CRC press.
 * 
 * @since 1.0
 * @author Aaron Garrett
 * @version 1.0
 */
public class BinaryGenerationalGA {
	 ...
	 */
	public BinaryGenerationalGA(int populationSize, int numGenes, int numElites, double crossoverUsageRate, double mutationUsageRate, double mutationRate, EvaluationFunction ef, TerminationCriteria tc, ECMonitor ecEval) {
	 ...
	 */
	public BinaryGenerationalGA(int populationSize, int numGenes, int numElites, double crossoverUsageRate, double mutationUsageRate, double mutationRate, EvaluationFunction ef, TerminationCriteria tc, ECMonitor ecEval, MigrationOperator migOp) {
		terminationCriteria = tc;
		ecEvaluator = ecEval;
		numDimensions = numGenes;
		Interval[] bounds = new Interval[numDimensions];
