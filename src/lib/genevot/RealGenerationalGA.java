...
/** This class represents a real-coded generational GA. This GA uses binary tournament selection, generational replacement (with possible elitism), BLX-alpha crossover, and Gaussian mutation. For a detailed treatment of GAs, see G. Dozier, A. Homaifar, E. Tunstel,  and D. Battle, "An Introduction  to Evolutionary Computation" (Chapter 17), Intelligent Control Systems Using Soft Computing Methodologies, A. Zilouchian & M. Jamshidi (Eds.), pp. 365-380, CRC press.
 * 
 * @since 1.0
 * @author Aaron Garrett
 * @version 1.0
 */
public class RealGenerationalGA {
	...
	/** This constructor creates an instance of the GA based on the population size, the array of minimum bounds for the genes, the array of maximum bounds for the genes, the number of elite individuals, crossover usage rate, the alpha value for BLX crossover, mutation usage rate, mutation rate, the mutation range, evaluation function, termination criteria, and EC evaluator. No migration is used.
	 * 
	 * @since 1.0
	 ...
	 */
	public RealGenerationalGA(int populationSize, double[] min, double[] max, int numElites, double crossoverUsageRate, double blxAlpha, double mutationUsageRate, double mutationRate, double mutationRange, EvaluationFunction ef, TerminationCriteria tc, ECMonitor ecEval) {
		...
		population = new Population(populationSize, bounds, ef, tournSelection, blxRecombination, gaussMutOp, genSelection, noMig);
	}
			
	/** This constructor creates an instance of the GA based on the population size, the array of minimum bounds for the genes, the array of maximum bounds for the genes, the number of elite individuals, crossover usage rate, the alpha value for BLX crossover, mutation usage rate, mutation rate, the mutation range, evaluation function, termination criteria, EC evaluator, and migration operator.
	 * 
	 * @since 1.0
	 * @param populationSize The size of the population
	 * @param min The array of minimum bounds for the genes
	 ...
	 */
	public RealGenerationalGA(int populationSize, double[] min, double[] max, int numElites, double crossoverUsageRate, double blxAlpha, double mutationUsageRate, double mutationRate, double mutationRange, EvaluationFunction ef, TerminationCriteria tc, ECMonitor ecEval, MigrationOperator migOp) {
		terminationCriteria = tc;
		ecEvaluator = ecEval;
		numDimensions = min.length;
		Interval[] bounds = new Interval[numDimensions];
		...
		//		         Population(int           , Interval[], EvaluationFunction, ParentSelection, RecombinationOperator, MutationOperator, SurvivorSelection, MigrationOperator)
		population = new Population(populationSize, bounds, ef, tournSelection, blxRecombination, gaussMutOp, genSelection, migOp);
	}
